...
import {bridgeDecision} from '../pipeline/bridge-adapters.mjs';
...
if(mode==='--worker'){
 const step=Number(process.argv[3]);assert.ok(Number.isInteger(step)&&step>=0&&step<=policy().controller.max_revisions);
 ...
 const adapters=resolveAdapters(p,{
  baselineSpec:read(`${root}/baseline_designspec.json`),firstRevisionSpec:read(`${root}/design_iteration_01.json`),
  revisionMetadata:({revision_step:step,previous:accepted,value:primaryScale})=>{
   const previous=accepted.data;
   const feedback={schema_version:'2.0.0',provenance:provenance('npm run closed-loop:bridge'),parent_design_id:previous.design_id,previous_response_sha256:previous.response_sha256,policy_sha256:hash(policyPath),baseline_reference:p.objective.reference_baseline,observed_deck_mm:previous.max_deck_displacement_mm,observed_reduction_percent_from_baseline:previous.decision.reduction_percent,target_reduction_percent:p.objective.target.value,target_interpretation:p.objective.target.interpretation,decision:previous.decision,parameter_update_rule:'primaryScale_next = primaryScale_current + 0.25',proposed_primaryScale:primaryScale,rationale:'Previous accepted response missed the illustrative target; apply the fixed increment, within the original supported range.',hypothesis_before_reanalysis:'Larger primary sections may reduce deck displacement; response and acceptance require the unchanged solver.',tradeoffs:['Greater primary-section/material demand; total structural mass and added self-weight are not modeled.','Frame/cable forces and cable states can redistribute; screening concerns may persist.'],reanalysis_required:true};
   const feedbackPath=`${out}/feedback_for_iteration_${pad(step)}.json`;write(feedbackPath,feedback);
   return {policy_sha256:hash(policyPath),feedback_reference:feedbackPath,feedback_sha256:hash(feedbackPath),provenance:provenance('npm run closed-loop:bridge')};
  }
 });
 const genericTrajectory=runController({policy:p.controller,initialDesign:read(`${root}/baseline_designspec.json`),...adapters,
 obtainState:({revision_step:step,design})=>{
  const primaryScale=adapters.designVariableAdapter.read(design);
  if(step>=2)write(specPath(step),design);
  else assert.deepEqual(design,read(specPath(step)),'Saved DesignSpec changed');
  ...
  return {response:record,validity:adapters.acceptance(record),data:state};
 },onEvaluated:(evaluated,allStates)=>{
  const state=evaluated.data,states=allStates.map(s=>s.data);
  state.decision=bridgeDecision(evaluated.decision);
  const base=states[0];
 ...
 const trajectory={...genericTrajectory,states:genericTrajectory.states.map(s=>s.data),final_stopping_reason:bridgeDecision(genericTrajectory.states.at(-1).decision).reason,illustrative_target_met:genericTrajectory.target_met,final_accepted_state:genericTrajectory.final_accepted_state?.data??null};
