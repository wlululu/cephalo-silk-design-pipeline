# Development and Scientific Provenance

This archive preserves the detailed root README from Design Pipeline commit `86f13f5b5019e937d3b224c1c51a141b1701ef7a` (`Extend Bridge pipeline with multi-step stopping rule`). It contains development history, scientific provenance, implementation notes, validation context, and historical standalone Structural Lab records.

The original content below is preserved, with only relative Markdown links adjusted for this document's location. Version terminology and statements about publication or uncommitted work describe the historical record, not the current repository status. Code-formatted paths are relative to the repository root. See the [public README](../README.md) for current project usage and the [pipeline documentation](../pipeline/README.md) for detailed controller and cross-platform reproducibility notes.
